...
// servicePortProvider and servicePortUpdater are this file's own stubs. Upstream
// shares recordingProvider and recordingUpdater across the reconciler tests; the
// copy of recordingProvider here counts Update calls, which the shared one does
// not, and there is no shared updater stub to reuse. Fold them together when the
// upstream test scaffolding is backported.
type servicePortProvider struct {
	updated   int
	deleted   []k8stypes.NamespacedName
	deleteErr error
}

func (p *servicePortProvider) Register(string, *http.ServeMux) {}

func (p *servicePortProvider) Update(context.Context, *provider.TranslateContext, client.Object) error {
	p.updated++
	return nil
}

func (p *servicePortProvider) Delete(_ context.Context, obj client.Object) error {
	p.deleted = append(p.deleted, k8stypes.NamespacedName{Namespace: obj.GetNamespace(), Name: obj.GetName()})
	return p.deleteErr
}

func (p *servicePortProvider) Start(context.Context) error { return nil }

func (p *servicePortProvider) NeedLeaderElection() bool { return true }

type servicePortUpdater struct {
	updates []status.Update
}

func (u *servicePortUpdater) Update(update status.Update) { u.updates = append(u.updates, update) }
...
func newServicePortFixture(
...
) (*ApisixRouteReconciler, *servicePortProvider, *servicePortUpdater) {
	t.Helper()
	...
	prov := &servicePortProvider{}
	updater := &servicePortUpdater{}
...
func acceptedCondition(t *testing.T, updater *servicePortUpdater) metav1.Condition {
	t.Helper()
