...
func (r *UDPRouteReconciler) listUDPRoutesForBackendTrafficPolicy(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	for _, targetRef := range policy.Spec.TargetRefs {
		...
		if err := r.List(ctx, udprList, client.MatchingFields{
			...
			r.Log.Error(err, "failed to list udproutes by service reference", "service", targetRef.Name)
			return nil
		}
...
func (r *UDPRouteReconciler) listUDPRoutesForGateway(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	if err := r.List(ctx, udprList, client.MatchingFields{
		...
		r.Log.Error(err, "failed to list udproutes by gateway", "gateway", gateway.Name)
		return nil
	}
...
// listUDPRoutesForGatewayProxy list all UDPRoute resources that are affected by a given GatewayProxy
func (r *UDPRouteReconciler) listUDPRoutesForGatewayProxy(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	for _, gateway := range gatewayList.Items {
		...
		if err := r.List(ctx, udpRouteList, client.MatchingFields{
			...
			r.Log.Error(err, "failed to list udproutes for gateway", "gateway", gateway.Name)
			continue
		}
...
func (r *UDPRouteReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.processUDPRouteBackendRefs(tctx, req.NamespacedName); err != nil && backendRefErr == nil {
		backendRefErr = err
	}
		...
		for _, backend := range rule.BackendRefs {
			...
			tctx.BackendRefs = append(tctx.BackendRefs, gatewayv1.BackendRef{
				BackendObjectReference: gatewayv1.BackendObjectReference{
					Name:      backend.Name,
					Namespace: cmp.Or(backend.Namespace, (*gatewayv1.Namespace)(&udpRoute.Namespace)),
					Port:      backend.Port,
				},
			})
		}
...
func (r *UDPRouteReconciler) processUDPRouteBackendRefs(tctx *provider.TranslateContext, trNN k8stypes.NamespacedName) error {
	var terr error
	for _, backend := range tctx.BackendRefs {
		...
		// if cross namespaces between UDPRoute and referenced Service, check ReferenceGrant
...
func (r *UDPRouteReconciler) listUDPRoutesByServiceRef(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	if err := r.List(ctx, trList, client.MatchingFields{
		...
		r.Log.Error(err, "failed to list udproutes by service", "service", serviceName)
		return nil
	}
...
func (r *UDPRouteReconciler) listUDPRoutesForL4RoutePolicy(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	for _, ref := range policy.Spec.TargetRefs {
		if string(ref.Group) != gatewayv1.GroupName || string(ref.Kind) != KindUDPRoute {
			continue
		}
