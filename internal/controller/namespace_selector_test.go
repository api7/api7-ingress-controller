...
// The stubs below are local to this file: this branch has no shared reconciler
// test scaffolding to reuse.
type nsSelectorProvider struct {
	updated int
	deleted []k8stypes.NamespacedName
}

func (p *nsSelectorProvider) Register(string, *http.ServeMux) {}

func (p *nsSelectorProvider) Update(context.Context, *provider.TranslateContext, client.Object) error {
	p.updated++
	return nil
}

func (p *nsSelectorProvider) Delete(_ context.Context, obj client.Object) error {
	p.deleted = append(p.deleted, k8stypes.NamespacedName{Namespace: obj.GetNamespace(), Name: obj.GetName()})
	return nil
}

func (p *nsSelectorProvider) Start(context.Context) error { return nil }

func (p *nsSelectorProvider) NeedLeaderElection() bool { return true }

type nsSelectorUpdater struct {
	updates []status.Update
}

func (u *nsSelectorUpdater) Update(update status.Update) { u.updates = append(u.updates, update) }

func nsSelectorScheme(t *testing.T) *runtime.Scheme {
	t.Helper()
...
func nsSelectorIngressClass() *networkingv1.IngressClass {
...
func nsSelectorReadier(t *testing.T, cli client.Client) readiness.ReadinessManager {
	t.Helper()
...
func TestIsWatchedNamespace(t *testing.T) {
	cli := fake.NewClientBuilder().WithScheme(nsSelectorScheme(t)).
...
func TestFindMatchingIngressClassByObject_NamespaceSelector(t *testing.T) {
	cli := fake.NewClientBuilder().WithScheme(nsSelectorScheme(t)).
		WithObjects(append(selectorNamespaces(), nsSelectorIngressClass())...).Build()
	setNamespaceSelector(t, "team=a")
	...
	ic, err := FindMatchingIngressClassByObject(context.Background(), cli, logr.Discard(), route(watchedNamespace), "")
	require.NoError(t, err)
	assert.Equal(t, "apisix", ic.Name)

	_, err = FindMatchingIngressClassByObject(context.Background(), cli, logr.Discard(), route(unwatchedNamespace), "")
...
func TestApisixRouteReconcile_RetractsOutsideWatchedNamespace(t *testing.T) {
	scheme := nsSelectorScheme(t)
	route := &apiv2.ApisixRoute{
		ObjectMeta: metav1.ObjectMeta{Namespace: unwatchedNamespace, Name: "route"},
		Spec:       apiv2.ApisixRouteSpec{IngressClassName: "apisix"},
	}
	cli := fake.NewClientBuilder().WithScheme(scheme).
		WithObjects(append(selectorNamespaces(), nsSelectorIngressClass(), route)...).
		WithStatusSubresource(route).
		Build()
	setNamespaceSelector(t, "team=a")

	prov := &nsSelectorProvider{}
	updater := &nsSelectorUpdater{}
	r := &ApisixRouteReconciler{
		...
		Provider: prov,
		Updater:  updater,
		Readier:  nsSelectorReadier(t, cli),
	}
...
func TestApisixTlsReconcile_RetractsOutsideWatchedNamespace(t *testing.T) {
	...
	cli := fake.NewClientBuilder().WithScheme(scheme).
		WithObjects(append(selectorNamespaces(), nsSelectorIngressClass(), tls)...).
