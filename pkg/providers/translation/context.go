...
import apisix "github.com/api7/api7-ingress-controller/pkg/types/apisix/v1"

// TranslateContext contains APISIX resources generated by the translator.
type TranslateContext struct {
	Routes        []*apisix.Route
	Services      []*apisix.Service
...
func (tc *TranslateContext) AddService(u *apisix.Service) {
	if _, ok := tc.ServiceMap[u.Name]; ok {
		return
	}
	tc.ServiceMap[u.Name] = struct{}{}
	tc.Services = append(tc.Services, u)
}

func (tc *TranslateContext) CheckUpstreamExist(name string) (ok bool) {
	_, ok = tc.ServiceMap[name]
	return
}
