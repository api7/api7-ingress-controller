...
`
	...
	It("recovers when the data plane holds newer conf_versions than the ADC cache", func() {
		...
		// The Deployer interface is shared with the API7 control plane, which has no
		// standalone Admin API, so the client lives on the APISIX deployer alone.
		deployer, ok := s.Deployer.(*scaffold.APISIXDeployer)
		Expect(ok).To(BeTrue(), "the standalone specs run on the APISIX deployer")
		admin := deployer.AdminAPIClient()
