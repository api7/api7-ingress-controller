...
func (f *Framework) deletePods(selector string) {
	...
	for _, pod := range podList.Items {
		_ = f.clientset.CoreV1().Pods(_namespace).Delete(f.Context, pod.Name, metav1.DeleteOptions{GracePeriodSeconds: ptr.To(int64(30))})
	}
}

func (f *Framework) applySSLSecret(name string, cert, pkey, caCert []byte) {
	kind := "Secret"
	apiVersion := "v1"
	...
	_, err := f.clientset.CoreV1().Secrets(_namespace).Apply(context.TODO(), &secret, metav1.ApplyOptions{
		FieldManager: "e2e",
	})
...
func (f *Framework) CreateNamespaceWithTestService(name string) {
	_, err := f.clientset.CoreV1().Namespaces().Create(f.Context, &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: name}}, metav1.CreateOptions{})
...
func (f *Framework) Scale(name string, replicas int32) {
	...
	_, err = f.clientset.AppsV1().Deployments(_namespace).UpdateScale(context.Background(), name, scale, metav1.UpdateOptions{})
	...
	f.GomegaT.Expect(err).ShouldNot(HaveOccurred(), fmt.Sprintf("ensure service %s/%s has %v endpoints failed", _namespace, name, replicas))
}
