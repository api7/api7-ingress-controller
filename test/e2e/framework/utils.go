...
func (f *Framework) LoginDashboardBySAML(loginPath string, username, password string, redirectURI string, acsErrmsg string) ([]*http.Cookie, []*http.Cookie) {
...
func (f *Framework) LogoutDashboardBySAML(logoutPath string, cookies []*http.Cookie, keycloakCookies []*http.Cookie, redirectURI string, logoutIDPSession bool, logoutErrMsg string) {
	client := http.DefaultClient
	client.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		return http.ErrUseLastResponse
	}
...
func (f *Framework) GetPodLogs(name string, previous bool) string {
	reader, err := f.clientset.CoreV1().Pods(_namespace).GetLogs(name, &v1.PodLogOptions{Previous: previous}).Stream(context.Background())
